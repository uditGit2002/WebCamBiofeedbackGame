...
public class Detector extends PApplet{
	int barWidth = 5;
	int lastBar = -1;
	...
	Mat image = new Mat(), motion = new Mat(), mhi=new Mat(), orient=new Mat(), mask =new Mat(), segmask=new Mat();
	Mat[] buf;
	...
	public Detector(PApplet pApplet) {
		...
		videoWidth = (int) capture.get(3);
		videoHeight = (int) capture.get(4);
